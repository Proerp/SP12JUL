...
namespace TotalDAL.Helpers.SqlProgrammability.Productions
{
    public class FinishedItem
    {
        ...
        private void GetFinishedItemPendingFirmOrders()
        {
            ...
            queryString = queryString + "       SELECT          FirmOrders.PlannedOrderID, FirmOrders.FirmOrderID, FirmOrders.EntryDate AS FirmOrderEntryDate, FirmOrders.Reference AS FirmOrderReference, FirmOrders.Code AS FirmOrderCode, FirmOrders.Specification AS FirmOrderSpecification, FirmOrders.TotalQuantity AS FirmOrderQuantity, ROUND(FirmOrders.TotalQuantity - (FirmOrders.TotalQuantitySemifinished - FirmOrders.TotalQuantityShortage - FirmOrders.TotalQuantityFailure + FirmOrders.TotalQuantityExcess), " + (int)GlobalEnums.rndQuantity + ") AS FirmOrderRemains, SemifinishedItemDetails.QuantityRemains, Customers.CustomerID, Customers.Code AS CustomerCode, Customers.Name AS CustomerName " + "\r\n";
            queryString = queryString + "       FROM           (SELECT FirmOrderID, ROUND(SUM(Quantity - QuantityFinished), " + (int)GlobalEnums.rndQuantity + ") AS QuantityRemains FROM SemifinishedItemDetails WHERE Approved = 1 AND HandoverApproved = 1 AND ROUND(Quantity - QuantityFinished, " + (int)GlobalEnums.rndQuantity + ") > 0 GROUP BY FirmOrderID) SemifinishedItemDetails " + "\r\n";//LocationID = @LocationID AND 
            queryString = queryString + "                       INNER JOIN FirmOrders ON SemifinishedItemDetails.FirmOrderID = FirmOrders.FirmOrderID " + "\r\n";
        ...
        private void FinishedItemSaveRelative()
        {
            ...
            queryString = queryString + "               SET     FirmOrders.TotalQuantityFinished = ROUND(FirmOrders.TotalQuantityFinished + (FinishedItemSummaries.TotalQuantity + FinishedItemSummaries.TotalQuantityFailure + FinishedItemSummaries.TotalQuantityShortage) * @SaveRelativeOption, " + (int)GlobalEnums.rndQuantity + "), " + "\r\n";
            queryString = queryString + "                       FirmOrders.TotalQuantityFailure = ROUND(FirmOrders.TotalQuantityFailure + FinishedItemSummaries.TotalQuantityFailure * @SaveRelativeOption, " + (int)GlobalEnums.rndQuantity + "), " + "\r\n";
            queryString = queryString + "                       FirmOrders.TotalQuantityExcess = ROUND(FirmOrders.TotalQuantityExcess + FinishedItemSummaries.TotalQuantityExcess * @SaveRelativeOption, " + (int)GlobalEnums.rndQuantity + "), " + "\r\n";
            queryString = queryString + "                       FirmOrders.TotalQuantityShortage = ROUND(FirmOrders.TotalQuantityShortage + FinishedItemSummaries.TotalQuantityShortage * @SaveRelativeOption, " + (int)GlobalEnums.rndQuantity + "), " + "\r\n";
            queryString = queryString + "                       FirmOrders.TotalSwarfs = ROUND(FirmOrders.TotalSwarfs + FinishedItemSummaries.TotalSwarfs * @SaveRelativeOption, " + (int)GlobalEnums.rndQuantity + ") " + "\r\n";
            queryString = queryString + "               FROM    FirmOrders INNER JOIN (SELECT FirmOrderID, SUM(Quantity) AS TotalQuantity, SUM(QuantityFailure) AS TotalQuantityFailure, SUM(QuantityExcess) AS TotalQuantityExcess, SUM(QuantityShortage) AS TotalQuantityShortage, SUM(Swarfs) AS TotalSwarfs FROM FinishedItemDetails WHERE FinishedItemID = @EntityID GROUP BY FirmOrderID) AS FinishedItemSummaries ON FirmOrders.FirmOrderID = FinishedItemSummaries.FirmOrderID ; " + "\r\n";
            queryString = queryString + "           END " + "\r\n";
            queryString = queryString + "       ELSE " + "\r\n";
            queryString = queryString + "           BEGIN " + "\r\n";
